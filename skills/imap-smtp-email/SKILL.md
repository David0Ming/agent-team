...
For multiple email accounts, create `accounts.json`:
...
When `accounts.json` exists, all commands will use these accounts instead of `.env` variables.
...
### filter
Smart filter emails by category (interview, resume, test, offer).

```bash
node scripts/filter.js [--account=<name>] [--days=7]
```

Options:
- `--account=<name>`: Filter specific account only
- `--days=<n>`: Check emails from last N days (default: 7)
...
Output: Markdown format with subject, sender, time, and account.

### summary
Generate daily email summary and auto-create todos.
...
Features:
- Creates `memory/daily/email-summary-YYYY-MM-DD.md` with categorized emails
- Auto-detects interview/test emails and extracts time info
- Updates `memory/projects.md` with todos:
  - Interview/Test → 工作代办
  - Study events → 学习代办
