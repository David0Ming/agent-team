...
async function connect() {
  ...
  return new Promise((resolve, reject) => {
    ...
    imap.once('ready', () => {
      // Send IMAP ID command only for 163.com servers
      const is163 = config.host && config.host.includes('163.com');
      
      if (is163 && typeof imap.id === 'function') {
        imap.id(IMAP_ID, (err) => {
          if (err) {
        ...
        // Skip ID command for non-163 servers
        resolve(imap);
      }
    });
...
function extractLinks(text, html) {
  ...
  if (text) {
    ...
    if (matches) matches.forEach(link => links.add(link));
  }
...
async function checkEmails(mailbox = DEFAULT_MAILBOX, limit = 10, recentTime = null, unreadOnly = false) {
  ...
  try {
    ...
    // Sort UIDs descending (newest first) and take only the limit
    const limitedUids = uids.sort((a, b) => b - a).slice(0, limit);

    // Fetch only the limited messages
    const fetchOptions = {
      bodies: [''],
      markSeen: false,
    };

    const messages = await searchMessages(imap, [['UID', limitedUids]], fetchOptions);
    ...
    for (const item of messages) {
      ...
      const emailDir = path.join(os.homedir(), '.openclaw/workspace/memory/emails', dateStr);
      ...
      const emailData = {
        ...
        text: email.text,
        html: email.html,
        links: extractLinks(email.text, email.html),
        attachments: email.attachments || [],
        flags: email.flags,
      };

      const filePath = path.join(emailDir, `${email.uid}.json`);
