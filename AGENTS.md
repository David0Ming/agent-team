...
- 持续任务的完成标准不是“后台已执行”，而是“任务委托方已收到本轮结果，并由委托方决定是否向泽钢交付”
- 定时任务、cron、子Agent、后台流程只能作为执行手段，不能替代任务委托链路中的显式回报
- 子Agent的直接汇报对象默认只能是任务委托方（通常是DJJ），不直接替代DJJ向泽钢交付最终结果；DJJ负责整合后再面向泽钢回报
- 若后台流程已跑但任务委托方未收到结果，DJJ必须主动补齐回流，直到形成闭环
- 若泽钢明确说“直接执行”“不要解释”，默认切换到少解释强交付模式：输出以结果、改进、下一步为主，减少过程说明
...
### OUTPUT FORMAT
**定期维护**：
- 每周日 20:00：清理未使用的文件，整理日志
- 每月 1 日 10:00：归档旧日志，评估知识卡片和技能价值
- 每季度首月 1 日 10:30（1月/4月/7月/10月）：评估 Areas 的价值，删除或合并低价值长期责任项
